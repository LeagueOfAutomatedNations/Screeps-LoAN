...
ADD CONSTRAINT alliances_pkey PRIMARY KEY (shortname);
...
ADD CONSTRAINT unique_screeps_id UNIQUE (screeps_id);
...
ALTER TABLE ONLY users
    ADD CONSTRAINT lnk_users_alliances FOREIGN KEY (alliance) REFERENCES alliances(shortname) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE ONLY rooms
    ADD CONSTRAINT lnk_users_rooms FOREIGN KEY (owner) REFERENCES users(id) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE;
